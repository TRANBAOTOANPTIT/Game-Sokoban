...
public final class Board extends JPanel implements ActionListener {
    ...
    private int count;
    private final ArrayList<char[][]> mapList = gatherAllMaps();

    private final JButton nextLvlBtn;
    private final JButton resLvlBtn;
    private final JButton quitLvlBtn;
    private final JButton undoLvlBtn;
     ...
     * @param isChallenger
     */
    public Board(Main Main, boolean isChallenger) {
        ...
        this.Main = Main;
        Board.isChallenger = isChallenger;
        ...
        this.Main.setOldMap(oldMap);

        prevMoveMaps = new ArrayList<>();
    ...
    private class TAdapter extends KeyAdapter {
        ...
        public void keyPressed(KeyEvent e) {
            ...
            if (key == KeyEvent.VK_Q) {
                ...
                setVisible(false);
                //Main.getEx().setVisible(false);
                Main.getMenuScreen().requestFocusInWindow();
                Main.getMenuScreen().setVisible(true);
                isFirstTime = 0;
            }
            ...
            if (key == KeyEvent.VK_R) {
                if (!isComplete) { //dont reset when game is complete
                    resetFlag = true;
                    map = duplicateMap(Main.getOldMap());
            ...
            if (key == KeyEvent.VK_E) {
                ...
                if (isComplete) {
                    ...
                    if (count >= mapList.size()) { //Not tutorial 
                        map = generator2();
                        Main.setOldMap(duplicateMap(map));
                        mapList.add(oldMap);
                        prevMoveMaps.clear();
